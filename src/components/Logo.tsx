...
const Logo: React.FC<LogoProps> = ({ onClick, className = '', size = 'md' }) => {
  const sizeClasses = {
    sm: 'h-6 md:h-8',
    md: 'h-8 md:h-10',
    lg: 'h-10 md:h-12'
  ...
  if (onClick) {
    return (
      <motion.button
        ...
        whileTap={{ scale: 0.95 }}
        onClick={onClick}
        className={`inline-flex items-center justify-center px-6 py-3 bg-white rounded-full border border-slate-200 shadow-sm hover:shadow-md transition-all duration-200 ${className}`}
      >
        {logoElement}
      </motion.button>
    )
  ...
  return (
    <div className={`inline-flex items-center justify-center px-6 py-3 bg-white rounded-full border border-slate-200 shadow-sm ${className}`}>
      {logoElement}
    </div>
  )
}
